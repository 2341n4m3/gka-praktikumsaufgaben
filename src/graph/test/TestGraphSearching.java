...
public class TestGraphSearching {
...

	{
		try {
			...
			undirected = newUndirectedReader()
					.read("src/graph/misc/graph2.gka");
			undirected2 = newUndirectedReader().read(
					"src/graph/misc/graph2.1.gka");
		} catch (Exception e) {
			e.printStackTrace();
		}
	}
	...
	public void testBreadth() {
		...
		PathFinder<UndirectedGraph<Vertex, DefaultEdge>, Vertex, DefaultEdge> undirectedFinder2 = PathFinders
				.newUndirectedBreadthPathFinder(undirectedCounter2);

		GraphPath<Vertex, DefaultEdge> directedPath = directedFinder.apply(
				directed, new Vertex("a"), new Vertex("g"));
		...
		GraphPath<Vertex, DefaultEdge> undirectedPath = undirectedFinder.apply(
				undirected, new Vertex("a"), new Vertex("h"));
		...
		GraphPath<Vertex, DefaultEdge> undirectedPath2 = undirectedFinder2
				.apply(undirected2, new Vertex("a"), new Vertex("o"));
		System.out.println("breadth undirect");
		System.out.println(undirectedCounter2);
		System.out.println(undirectedPath2);
		System.out.println(undirectedPath2.getWeight());
	...
	private <E> void check(GraphPath<Vertex, E> path, List<String> vertices) {
		...
		assertEquals("checke ersten Knoten", expectedStartVertex, path
				.getStartVertex().getName());

 ...
 * PathFinder sind dazu gedachteinen 
 * kürzesten Pfad im Graph zu finden.
 * 
 ...
 * Zum Iterieren wird ein GraphIterator benutzt.
 * Die Nachbarn/Nachfolger des aktuellen Vertex werden über die findNeighbors Funktion ermittelt
...
public class PathFinder<G extends Graph<V, E>, V, E> {
	...
	public GraphPath<V, E> apply(G graph, V start, V end) {
		...
		vertexDistances.put(start, 0);
		 ...
		 /** Schleife laeuft solange bis der Iterator nur noch einen naechsten Knoten hat und
		 * finish true ist also der Endknoten markiert wurde.
		 */
		while (iterator.hasNext() && !finished) {
			...
			*Ist Nachbar nummeriert wird geprueft ob die neue Zahl kleiner als die jetzige Zahl ist.
			*Wenn ja erhaelt der Knoten eine neue Nummerierung, 
			*sonst erhaelt es zum ersten mal eine Nummerierung.
			*/
		...
		//Gibt ein Pfad zurueck der den kuerzestens Weg von Start- zu Zielvertex repräsentiert.
